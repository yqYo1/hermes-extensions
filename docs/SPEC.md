...
| `git\s+push\s+(-f\|--force)` | デフォルトブランチへのforce push禁止 |

**注意**: `git push --force`のブロック対象ブランチは、gitコマンドで動的に取得したリモートのデフォルトブランチ（`origin/HEAD`の参照先）を使用します。決め打ち（main/master/develop等）ではありません。
...
```python
def get_default_branch(remote_url: str = None) -> str:
    """リモートのデフォルトブランチを取得
    
    git remote show origin または git symbolic-ref refs/remotes/origin/HEAD
    を使用して動的に取得します。
    """
    import subprocess
    ...
    # フォールバック: 一般的なデフォルトブランチ名
    return "main"
```

### 5.3 git initの実効ディレクトリ計算
