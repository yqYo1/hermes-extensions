...
name: loop-detector
version: 2.0.0
description: "Detects LLM thinking loops and tool-call loops, blocks them, and notifies the model via ephemeral context injection."
author: yqYo1
license: MIT
hooks:
  - pre_tool_call
...
2. **思考ループ（Thinking Loop）**: 同じ内容や同じ推論パターンを繰り返し出力する LLM の振る舞い
...
- **検知は軽量に**: ツールループは完全一致ベースで検知する。類似度計算は思考ループに限定する
...
| 検出パターン | 検出されたループを一意に表す識別子。ツールループでは正規化ツール呼び出し（交互パターンでは周期シーケンス）、思考ループでは「thinking」固定 |
...
記録する。`post_llm_call` はターンにつき 1 回、ツール呼び出しループ完了後に発火する。
中断（interrupted）されたターンでは発火しない点に注意する。

### 5.2 検出条件

1. 各 `assistant_response` を正規化する（小文字化・空白畳み込み・数値の `{NUM}` 置換・
   コードフェンス言語指定の除去）
2. 直近 `window_size`（既定 5）件について隣接ペアの類似度を
   `difflib.SequenceMatcher.ratio()` で計算
3. 類似度が `similarity_threshold`（既定 0.85）以上のペアが `min_repetitions`
   （既定 2）組以上連続 → 検出
...
思考ループはブロック対象ではない（ブロック可能なゲートが `pre_tool_call` のみであるため）。
...
回復通知は**ブロックまたは思考ループ検出が発生したターンの次ターン**の `pre_llm_call` で
...
| `blocked_this_turn` | 現ターンでブロック済みの検出パターン（再確認抑止用） | ターン終了時クリア |

### 9.2 状態のリセット

- `on_session_reset` フック（CLI `/new`・`/reset`、ゲートウェイ `/new` で発火）で
  セッション状態を削除する
- `on_session_end` は使用しない。メッセージングゲートウェイでは発火しないため、
  状態クリーンアップの契機として信頼できない。状態はメモリ内のみで、プロセス終了で
  自然消滅する
...
plugins:
  loop_detector:
    ...
    thinking_loop:
      ...
      window_size: 5                  # 比較対象の直近応答数
      min_repetitions: 2              # 検出に必要な連続類似ペア数
    confirmation:
...
| `post_llm_call` | `session_id`, `task_id`, `turn_id`, `user_message`, `assistant_response`, `conversation_history`, `model`, `platform` | 思考ループ検出（`assistant_response` 文字列を記録・比較） |
...
- `post_llm_call` は中断されたターンでは発火しない
...
├── detector.py          # 検出アルゴリズム（ツールループ・思考ループ）
...
2. **思考ループはブロックできない**: ブロック可能なゲートは `pre_tool_call` のみであり、
   LLM 応答自体を差し止める機構はプラグインに存在しない。思考ループへの対応は
   回復通知による間接的な抑止となる
